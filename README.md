...
6) General:
  ...
  - Для каждого сервиса должен быть настроен CI/CD на GitHub actions / GitLab CI/CD / Bitbucket Pipelines со степами: 1) тесты, 2) линтер (golangci-lint), 3) линтер для протофайлов, 4) vulncheck, 5) билд образа и залив на свой dockerHub (в мастер ветке).  
  - Driver service должен быть залит на Gitlab, Order service на Bitbucket, остальные на Github. (Все репозитории должны быть приватными!)
