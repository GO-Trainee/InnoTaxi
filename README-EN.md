...
9) User can delete their profile (use soft delete).
...
11) If there are many wallets available for the User, User can choose from which balance will be drained.
...
2) Driver can sign in.
    - Fields for signing in: phone number, password.
3) Driver can change their status when trip is over. At the same time order's status changes to finished.
...
5) Driver can view their rating. Rating calculates on 20 previous trips.
6) Driver can view their trips (taxi type, user, from to).
7) Driver can have 2 statuses: busy, free.

Order Service:
1) Order Service acts as an orchestration  service for both User and Driver service. It is used for order processing. 
...
4) Order's statuses coul be in progress or finished.
...
3) Analyst can view the rating of all users.
...
Adding new services is allowed, it is not allowed to reduce the number of services. When you change the scheme (adding new services) application must be supplemented with a diagram and put it in the repository in .png and .drawio formats.
...
- When adding a new feature, refactoring or fixing bugs, it is necessary to branch from the Dev branch and create a new one. Branch naming must enclude Jira task ID.
- Do the work in the created branches and create a Pull Request in the Dev branch. Pull request's name must contain Jira task ID .
...
- If mentor will leave some fix notes in PR's comments, you should fix them and commit to the same branch.
- When PR is approved you should merge it to dev branch. Before merging all comits must be squashed.
...
- Front-end: Use Vue.js (3.0) composition API. Create registration form, authorization form, view profile, change fields in profile, delete profile. Use components, Pinia.
- VCS: GitHub; CI/CD: Github Actions.

3) Driver service:
- Handler layer must be generated out of a Swagger file. (No handmade HTTP interaction).
- MongoDB as DB.
- The database stores: information about Driver (name, phone number, email, password, taxi type), information about the last 20 orders, driver's rating as an array, driver's balance (earnings). When the length of the array with orders becomes >20, it is necessary to delete the irrelevant ones.
- Aggregation of the driver rating based on the last 20 ratings.
- Pprof handlers must be added (and used). Or another type of profiling if not GO is used.
- Front-end: Use Angular (latest version), create registration form, authorization and profile view, change fields in profile, delete profile.
- VCS: Gitlab; CI/CD: Gitlab CI/CD;

4) Order service:
- GraphQL as transport layer.
- Endpoint to search by fields: DriverID, UserID, from, to, fromDate, toDate. Implement partial search (when some fields are not specified - should be ignored).
- Add pagination for the given endpoint. Fields offset; limit.
- Elasticsearch for order service (ability to search **from** or **to** created trips). Search by prefix, full text search by trip comments, add possibility to search with transliteration and lexical errors.
- Additional task: to implement the possibility of selecting drivers based on user rating.
- Front-end: Use React + redux/redux toolkit. Create main page (front to GQL endpoints) with filters (from - to; fromDate - toDate; User name; Driver name; taxi type etc.).
...
- The entire application should be runned via docker-compose.
- All variables to be changed (database connection, user's free driver wait time) must be set via environment variables.
- Each service must have a README (describing what is needed to start it, environment variables) and a swagger (detailing all the endpoints).
- All services must be covered by integration tests and Unit tests.
- A CI/CD must be set up for each service with nex steps: 1) tests, 2) linter, 3) linter for protofiles, 4) vulncheck, 5) build image and upload to your dockerHub (in the master branch).
- Authentication using JWT.

  Go:
- UserService: Gin as HTTP-library. Analyst: Fiber.
- Usage of golangci-lint
- To work with Postgres it is allowed to use pure SQL or sqlc, sqlx, squirrel. Using ORM is not allowed.
- To implement the user's waiting for free drivers, use goroutines and channels, if necessary sync, x/sync packets.
